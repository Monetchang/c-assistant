...
class Supervisor(AgentBase):
    ...
    def planning(self, state: AgentState) -> Dict[str, Sequence[AnyMessage]]:
        """
        规划节点，处理复杂任务规划
        ...
        try:
            ...
            if not user_input:
                return {"messages": [AIMessage(content="No user input found", name="planning")]}
            
            # 创建规划代理并执行任务
            planning_agent = PlanningAgent()
            
            # 首先进行任务规划
            plan_result = planning_agent.plan_task(str(user_input))
            
            if plan_result["status"] == "success" and plan_result["plan"]:
                # 执行计划
                execution_result = planning_agent.execute_plan(plan_result["plan"])
                
                if execution_result["status"] == "completed":
                    # 生成最终回复
                    final_response = self._generate_planning_response(execution_result, str(user_input))
                else:
                    final_response = f"任务执行失败: {execution_result.get('error', '未知错误')}"
            else:
                final_response = f"任务规划失败: {plan_result.get('error', '未知错误')}"
            
            return {"messages": [AIMessage(content=final_response, name="planning")]}
            
        except Exception as e:
            error_message = f"规划过程中出现错误: {str(e)}"
            return {"messages": [AIMessage(content=error_message, name="planning")]}
    
    def _generate_planning_response(self, execution_result: Dict[str, Any], original_task: str) -> str:
        """
        根据执行结果生成最终回复
            ...
            for step_name, step_result in results.items():
                if step_result.get("status") == "success":
                    if "output" in step_result:
                        response_parts.append(f"**{step_name}**: {step_result['output']}\n")
                    elif "results" in step_result:
                        response_parts.append(f"**{step_name}**: 搜索完成，找到相关信息\n")
                else:
                    response_parts.append(f"**{step_name}**: 执行失败 - {step_result.get('error', '未知错误')}\n")
